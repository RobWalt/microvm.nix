...

{
  config = lib.mkIf config.microvm.guest.enable {
    ...
    boot.initrd.kernelModules =
      ...
      ++ lib.optionals
        (pkgs.targetPlatform.system == "x86_64-linux" && config.microvm.hypervisor == "firecracker")
        [
          # Keyboard controller that can receive CtrlAltDel
          "i8042"
        ]
      ++ lib.optionals (config.microvm.writableStoreOverlay != null) [ "overlay" ];

    microvm.kernelParams = config.boot.kernelParams ++ [ "init=${config.system.build.toplevel}/init" ];
