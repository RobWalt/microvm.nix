...
let
  ...
  machine =
    {
      x86_64-linux =
        if requirePci then throw "PCI configuration for stratovirt is non-functional" "q35" else "microvm";
      aarch64-linux = "virt";
    }
    .${system};
  ...
  devType =
    ...
    if requirePci then
      if addr < 32 then
        "pci,bus=pcie.0,addr=0x${pkgs.lib.toHexString addr}"
      else
        throw "Too big PCI addr: ${pkgs.lib.toHexString addr}"
    else
      "device";
            ...
            (
              ...
              [
                ...
                (lib.concatStringsSep "," (
                  ...
                  ++ lib.optionals (type == "tap") [ "ifname=${id}" ]
                  ++ lib.optionals (type == "macvtap") [ "fd=${toString macvtapFds.${id}}" ]
                  ++ lib.optionals tapMultiQueue [ "queues=${toString vcpu}" ]
                ))
                # TODO: devType (0x10 + i)
                "-device"
                (lib.concatStringsSep "," [
