...
let
  ...
  enableLibusb =
    ...
    pkg.overrideAttrs (
      oa: {
        configureFlags = oa.configureFlags ++ [ "--enable-libusb" ];
        buildInputs = oa.buildInputs ++ (with pkgs; [ libusb ]);
      }
    );

  minimizeQemuClosureSize =
    pkg:
    (pkg.override (
      oa: {
        # standin for disabling everything guilike by hand
        nixosTestRunner = if graphics.enable then oa.nixosTestRunner or false else true;
        enableDocs = false;
      }
    )).overrideAttrs
      (
  ...
  qemu = overrideQemu (
    if microvmConfig.cpu == null then pkgs.qemu_kvm else pkgs.buildPackages.qemu_full
  );
  ...
  requireUsb = graphics.enable || lib.any ({ bus, ... }: bus == "usb") microvmConfig.devices;
  ...
  accel = if microvmConfig.cpu == null then "kvm:tcg" else "tcg";

  # PCI required by vfio-pci for PCI passthrough
  pciInDevices = lib.any ({ bus, ... }: bus == "pci") devices;

  requirePci = graphics.enable || (!lib.hasPrefix "microvm" machine) || shares != [ ] || pciInDevices;

  machineOpts =
    if microvmConfig.qemu.machineOpts != null then
      microvmConfig.qemu.machineOpts
    else
      {
        x86_64-linux =
          {
  ...
  machineConfig = builtins.concatStringsSep "," (
    [ machine ] ++ map (name: "${name}=${machineOpts.${name}}") (builtins.attrNames machineOpts)
  );
  ...
  enumerate =
    ...
    if xs == [ ] then
      ...
      [ (builtins.head xs // { index = n; }) ] ++ (enumerate (n + 1) (builtins.tail xs));

  canSandbox =
    # Don't let qemu sandbox itself if it is going to call qemu-bridge-helper
    !lib.any ({ type, ... }: type == "bridge") microvmConfig.interfaces;
  ...
  kernelConsole =
    if microvmConfig.qemu.serialConsole == false then
      ""
...
in
{
  ...
  command = lib.escapeShellArgs (
    ...
    ++ lib.optionals (microvmConfig.cpu == null) [ "-enable-kvm" ]
    ++ cpuArgs
    ++ lib.optionals (system == "x86_64-linux") [
      ...
      "-append"
      "${kernelConsole} reboot=t panic=-1 ${toString microvmConfig.kernelParams}"
    ]
    ...
    ++ lib.optionals storeOnDisk [
      "-drive"
      "id=store,format=raw,read-only=on,file=${storeDisk},if=none,aio=io_uring"
      "-device"
      "virtio-blk-${devType},drive=store${lib.optionalString (devType == "pci") ",disable-legacy=on"}"
    ]
    ++ (
    ...
    ++ lib.optionals (shares != [ ]) (
      [
        "-object"
        "memory-backend-memfd,id=mem,size=${toString (mem + balloonMem)}M,share=on"
        "-numa"
        "node,memdev=mem"
      ]
      ++
          ...
          (
            ...
            }:
            {
              "virtiofs" = [
                ...
                "-device"
                "vhost-user-fs-${devType},chardev=fs${toString index},tag=${tag}"
              ];
              "9p" = [
                "-fsdev"
                "local,id=fs${toString index},path=${source},security_model=${securityModel}"
                "-device"
                "virtio-9p-${devType},fsdev=fs${toString index},mount_tag=${tag}"
              ];
            }
            .${proto}
          )
    ...
    ++
      lib.warnIf (forwardPorts != [ ] && !builtins.any ({ type, ... }: type == "user") interfaces)
            ...
            (
              ...
              [
                ...
                (lib.concatStringsSep "," (
                  ...
                  ++ lib.optionals (type == "user" && forwardPorts != [ ]) [ forwardingOptions ]
                  ...
                  ++ lib.optionals (type == "macvtap") [
                    (
                      let
                        fds = macvtapFds.${id};
                      in
                      if builtins.length fds == 1 then
                        "fd=${toString (builtins.head fds)}"
                      else
                        "fds=${lib.concatMapStringsSep ":" toString fds}"
                    )
                  ]
                  ++ lib.optionals (type == "tap" && tapMultiQueue) [ "queues=${toString vcpu}" ]
                ))
                "-device"
                "virtio-net-${devType},netdev=${id},mac=${mac}${
                  # romfile= does not work with x86_64-linux and -M microvm
                  # setting or -cpu different than host
                  lib.optionalString (requirePci || (microvmConfig.cpu == null && system != "x86_64-linux"))
                    ",romfile="
      ...
      builtins.concatMap
        (
          { bus, path, ... }:
          {
            pci = [
              "-device"
              "vfio-pci,host=${path},multifunction=on"
            ];
            usb = [
              "-device"
              "usb-host,${path}"
