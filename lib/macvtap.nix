...
let
  ...
  macvtapInterfaces = builtins.concatLists (
    lib.imap0
      (
        interfaceIndex: interface:
        builtins.genList
          (queueIndex: interface // { fd = interfaceFdOffset + interfaceIndex * queueCount + queueIndex; })
          queueCount
      )
      (builtins.filter ({ type, ... }: type == "macvtap") microvmConfig.interfaces)
  );
in
{
  openMacvtapFds =
  ...
  macvtapFds =
    builtins.foldl' (result: { id, fd, ... }: result // { ${id} = (result.${id} or [ ]) ++ [ fd ]; })
