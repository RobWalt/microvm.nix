...
rec {
  ...
  withDriveLetters =
    ...
    map ({ fst, snd }: fst // { letter = snd; }) (
      nixpkgs-lib.zipLists volumes (nixpkgs-lib.drop offset nixpkgs-lib.strings.lowerChars)
    );
  ...
  createVolumesScript =
    ...
    pkgs.lib.concatMapStringsSep "\n" (
      ...
      pkgs.lib.warnIf (label != null && !autoCreate)
          ...
          let
            labelOption =
              if autoCreate then
                ...
                null;
            labelArgument = if (labelOption != null && label != null) then "${labelOption} '${label}'" else "";
          in
