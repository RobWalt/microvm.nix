...
let
  variants = [
    ...
    [
      {
        id = "qemu";
        modules = [ { microvm.hypervisor = "qemu"; } ];
      }
      {
        id = "qemu-tcg";
        modules =
          let
            ...
            guestSystem = if "${system}" == "x86_64-linux" then "aarch64-unknown-linux-gnu" else "x86_64-linux";
          in
          [
            {
              microvm = {
                ...
                # (TCG)
                cpu = if "${system}" == "x86_64-linux" then "cortex-a53" else "Westmere";
              };
      ...
      {
        id = "crosvm";
        modules = [ { microvm.hypervisor = "crosvm"; } ];
      }
      {
        id = "firecracker";
        modules = [ { microvm.hypervisor = "firecracker"; } ];
      }
      {
        id = "kvmtool";
        modules = [ { microvm.hypervisor = "kvmtool"; } ];
      }
    ]
    # ro-store
    [
    ...
    [
      {
        # no
        id = null;
        modules = [ { boot.initrd.systemd.enable = false; } ];
      }
      {
        id = "systemd";
        modules = [ { boot.initrd.systemd.enable = true; } ];
      }
    ]
    # hardened profile
    [
        ...
        if variants == [ ] then
          ...
          builtins.concatMap
            (
              ...
              in
              if tail == [ ] then [ [ head ] ] else map (t: [ head ] ++ t) tail
            )
  ...
  makeTestConfigs =
    ...
    builtins.foldl'
      (
                  ...
                  { lib, ... }:
                  {
                    options.microvm.testing.enableTest = lib.mkOption {
                      type = lib.mkOptionType {
                        ...
                        merge = loc: defs: builtins.all (def: def.value) defs;
                      };
                      default = true;
                    };
                  }
        ...
        in
        result
        // nixpkgs.lib.optionalAttrs nixOS.config.microvm.testing.enableTest { ${configName} = nixOS; }
      )
      { }
      allVariants;
...
//
  ...
  builtins.foldl'
    (
      ...
      in
      result // import ./vm.nix args // import ./iperf.nix args
    )
    { }
